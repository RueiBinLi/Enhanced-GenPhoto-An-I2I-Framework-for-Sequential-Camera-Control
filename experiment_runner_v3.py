...
# ==============================================================================
# 2. Embedding 生成邏輯 (嚴格參照 inference_multi_camera.py)
# ==============================================================================
...
def load_models_inversion(cfg, device):
    """載入 VAE, TextEncoder, UNet, Adapter 並強制讀取權重"""
...
class ExperimentRunner:
    ...
    def process_batch_i2i(self, pipeline, image_input, prompt, param_list, setting_type):
        ...
        target_emb_raw = target_embedder.load() # [F, C, H, W]
        target_emb_3d = rearrange(target_emb_raw, "f c h w -> 1 c f h w")

        # 2. 準備 Input Image for Inversion
    ...
    def run_stage_1(self):
        ...
        for setting_name, info in SETTINGS_CONFIG.items():
            ...
            setting_type = info['type_key'] # e.g. 'bokeh'
            ...
            for item in tqdm(data, desc=f"Stg1 {setting_name}"):
                ...
                if base_img_path:
                    ...
                    base_name = f"sample_{len(meta_baseline)}"
            ...
            with open(os.path.join(out_ours, "metadata.json"), "w") as f:
                json.dump(meta_ours, f, indent=4)
            
            del pipeline
            torch.cuda.empty_cache()

    def build_image_pool(self, source_dir_root, stage_prefix="stage1_baseline"):
        pool = []
        pattern = os.path.join(source_dir_root, f"{stage_prefix}_*")
        dirs = glob.glob(pattern)
        logger.info(f"Building Pool from: {pattern} -> Found {len(dirs)} dirs")
        ...
        for d in dirs:
            ...
            if os.path.exists(meta_path):
                with open(meta_path, 'r') as f:
                    items = json.load(f)
                    for item in items:
                        item['_abs_path'] = os.path.join(d, item['filename'])
                    pool.extend(items)
        return pool
...
'''
python experiment_runner_v3.py \
  --stage 1 \
