...
class ExperimentRunner:
    ...
    def save_split_frames(self, video_tensor, metadata_base, save_dir, param_list, setting_key, base_name):
        ...
        for i, frame in enumerate(video_tensor):
            ...
            # [註記] 這裡將 setting_key (e.g. 'bokeh') 存入 history
    ...
    def run_stage_1(self):
        logger.info("=== Starting Stage 1 (All Settings) ===")
        for setting_name, info in SETTINGS_CONFIG.items():
            ...
            for item in tqdm(data, desc=f"Stg1 {setting_name}"):
                ...
                if base_img_path and os.path.exists(base_img_path):
                    ...
                    logger.warning(f"⚠️ Skipping Stage 1 I2I: Input image not found at '{base_img_path}'")
    ...
    def build_image_pool(self, source_dir_root, stage_prefix):
        pool = []
        pattern = os.path.join(source_dir_root, f"{stage_prefix}_*")
        dirs = glob.glob(pattern)
        logger.info(f"Building Pool from pattern: {pattern}")
        ...
        for d in dirs:
            ...
            if os.path.exists(meta_path):
                with open(meta_path, 'r') as f:
                    items = json.load(f)
                    for item in items: item['_abs_path'] = os.path.join(d, item['filename'])
                    pool.extend(items)
        return pool
    ...
    def run_stage_sequential(self, stage_num):
        ...
        # [修正] Stage 3+ 讀取的是 stage{N-1}_* 而非 baseline
        if stage_num == 2: prev_stage_prefix = "stage1_baseline"
        else: prev_stage_prefix = f"stage{stage_num-1}"

        pool = self.build_image_pool(self.args.output_dir, prev_stage_prefix)
        if not pool: raise ValueError(f"Pool empty! Could not find folders starting with '{prev_stage_prefix}' in {self.args.output_dir}")

        for setting_name, info in SETTINGS_CONFIG.items():
            logger.info(f"--- Stage {stage_num}: {setting_name} ---")
            cfg = OmegaConf.load(info['config'])
            pipeline = load_models_inversion(cfg, self.device)
            setting_type = info['type_key'] # 這是重點，例如 'bokeh'

            with open(info['json'], 'r') as f: json_data = json.load(f)
            
            out_dir = os.path.join(self.args.output_dir, f"stage{stage_num}_{setting_name}")
            os.makedirs(out_dir, exist_ok=True)
            new_metadata = []

            for item in tqdm(json_data, desc=f"Stg{stage_num} {setting_name}"):
                try: param_list = json.loads(item[info['list_key']])
                except: continue
                
                # [修正] 過濾機制：檢查 history 是否包含當前的 setting_type ('bokeh' not 'bokehK')
                candidates = []
                for p_item in pool:
                    hist_keys = []
                    # history 結構是 [{'bokeh': val}, {'shutter': val}]
                    for h in p_item.get('history', []): hist_keys.extend(h.keys())
                    
                    if setting_type not in hist_keys:
                        candidates.append(p_item)
                
                if not candidates:
                    logger.warning(f"  No suitable non-overlapping images for {setting_name}. Skipping this batch.")
                    continue
...
'''
python experiment_runner_v4.py \
  --stage 1 \
